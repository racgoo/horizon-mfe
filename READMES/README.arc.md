...
> **⚠️ قد يتعارض CSS.** تُحقَن أنماط التطبيق الفرعي في المستند وقد تتعارض مع المضيف أو التطبيقات الأخرى. لتجنب التعارض **يجب** استخدام أحد الخيارات: **(1) Shadow DOM** (`<horizon-app>`)، **(2) تحديد نطاق كل المحددات** بـ `[data-horizon-app="<name>"]`، أو **(3) أسماء classes مهاشة/محددة النطاق** (مثل CSS Modules). لا تعتمد فقط على أنماط عامة غير محددة النطاق.
