...
(window as unknown as { navigate: (path: string) => void }).navigate = navigateTo;
...
function syncActiveLink() {
  document.querySelectorAll<HTMLAnchorElement>("nav a[data-link]").forEach((a) => {
...
eventBus.on<number>("store:count", (value) => {
  document.getElementById("event-log-value")!.textContent = `count → ${value}`;
});
