...
> **⚠️ CSS가 충돌할 수 있습니다.** 차일드 앱 스타일은 문서에 주입되며 호스트나 다른 앱과 겹칠 수 있습니다. 충돌을 피하려면 반드시 다음 중 하나를 사용하세요: **(1) Shadow DOM** (`<horizon-app>`), **(2) 모든 선택자에** `[data-horizon-app="<name>"]` **스코프 지정**, **(3) 해시/스코프된 클래스명** (예: CSS Modules). 전역·비스코프 스타일만 의존하지 마세요.
