...
> **⚠️ CSS の衝突に注意。** 子アプリのスタイルは document に注入されるため、ホストや他アプリと衝突する可能性があります。衝突を防ぐには次のいずれかが**必須**です：**(1) Shadow DOM**（`<horizon-app>`）、**(2) すべてのセレクターを** `[data-horizon-app="<name>"]` **でスコープ**、**(3) ハッシュ/スコープ付きクラス名**（例：CSS Modules）。グローバルでスコープなしのスタイルだけに頼らないでください。
